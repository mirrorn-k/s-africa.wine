...
# Composer v1 を明示インストール（1.10.26をピン留め）
RUN set -eux; \
  php -r "copy('https://getcomposer.org/installer','/tmp/composer-setup.php');" \
  && php /tmp/composer-setup.php --version=1.10.26 --install-dir=/usr/bin --filename=composer \
  ...
  && find ${APACHE_DOCUMENT_ROOT} -type d -print0 | xargs -0 chmod g+s

USER www-data

# ここで“プラグインを無効”にしてコアだけ入れる（昔はここで落ちてた）
RUN composer install --no-dev --no-scripts --no-plugins -d ${APACHE_DOCUMENT_ROOT}
